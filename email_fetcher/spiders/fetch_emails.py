...
class EmailFetcherSpider(scrapy.Spider):
    name = 'fetch_emails'
    def __init__(self, start_url=None, country=None, 
    ...
    def parse(self, response):
        ...
        for company in company_block:
            ...
            yield response.follow(website_link, callback=self.parse_emails, meta={'company_name': company_name, 'is_homepage': True})
        ...
        if is_homepage:
            for link in set(response.css("a::attr(href)").getall()):
                ...
                if "contact" in link_lower or "about" in link_lower and not link.startswith("mailto:") and not link.startswith("tel:"):
                    yield response.follow(link, callback=self.parse_emails, meta={'origin_url': response.url, 'company_name': company_name})
